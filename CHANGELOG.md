...
更新元|導入手順
---|---
0.2beta13|1. `PSDToolKit\口パク準備 設定上書き.exa` を削除<br>2. 全てのファイルを上書き
それ以前|上書きできないため、以前導入した PSDToolKit を削除してください。

- アニメーション効果でのレイヤー切り替え時の挙動をレイヤークリックと同等に変更
  - レイヤーを表示しようとした時は、親階層の全てのレイヤーを全て表示へ切り替えます
- `PSD ファイルオブジェクト` に `ls_locut` など `口パク準備` の設定省略時のデフォルト値を追加
- `口パク準備` の `ローカット` などをオプション設定に変更し、初期値を `0`（未指定）に変更
- `口パク準備 設定上書き` を廃止
  - 同等の処理を `PSD ファイルオブジェクト` で行えます
  - `口パク準備 設定上書き` を使っているプロジェクトは、`PSD ファイルオブジェクト` を作り直すことで対応できます
- `口パク 開閉のみ` に `感度` パラメーターを追加
- `動画ハンドル開放` プラグインを同梱
