...
- 小数点の区切り文字が `.` でない環境で使用できなかった問題を修正
- PSDファイル名に `[` と `]` が含まれていると LuaJIT 環境でエラーになっていたのを修正
  - バグ修正の影響で非 LuaJIT 環境でも同様のファイルが一時的に表示されなくなりますが、ドロップし直したりすることで正常に動作するはずです
- バージョン表記を2桁から3桁に変更(v0.2 → v0.2.0)した
- ごちゃまぜドロップスを v0.4.0beta2 に更新
- AudioMixer.auf を単体配布版 v0.3.0beta3 を同梱する形に変更
  - v0.3.0 系は一部に仕様変更を含みます。
  - 今までは PSDToolKit と同じバージョン番号を付与していましたが、今後は別に管理されます。
