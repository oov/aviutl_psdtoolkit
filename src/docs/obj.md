...
`wordwrap.mode`|テキストの自動改行に関する設定です。<br>`0` - 自動改行しない<br>`1` - 自動改行する<br>`2` - 禁則処理付きで自動改行する<br>`3` - 文節を考慮して禁則処理付きで自動改行する
`wordwrap.width`|自動改行の幅を指定します。<br>`wordwrap.mode` が `0` の場合は無視されます。
`wordwrap.model`|使用する BudouX モデルを指定します。<br>`"ja"` - 日本語モデル<br>`JSONファイルへのパス` - カスタムモデルを読み込みます。<br>※ この設定は `wordwrap.mode = 3` のときのみ有効です。

## テキストの自動改行について

[`字幕表示`](#字幕表示) や [`字幕表示（キャッシュ）`](#字幕表示（キャッシュ）) では、`wordwrap.mode` を `1` 以上にすることでテキストの自動改行を有効化することができます。

自動改行が有効なときに限り、以下のように `<nobr>` タグを使うことで改行禁止区間を指定することができます。
...
例えばテキストオブジェクトで以下のように記述すると、字幕表示ではない場所でも自動改行を行うことができます。

```
<?s,o=[==[
こんにちは
]==],{mode = 3, width = 800}
mes(require("PSDToolKit").wordwrap(s,o))?>
```
...
require("CacheText").mes(require("PSDToolKit").wordwrap(s,o),m)?>
```
