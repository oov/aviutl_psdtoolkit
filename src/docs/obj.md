...
`modifier`|文字列か関数を指定すると、字幕のテキストを改変できます。<br>文字列を指定した場合はテキストが先頭に追加され、関数を指定した場合は戻り値が字幕になります。

上記以外の設定項目や効果については [テキストの装飾](decorate.md) を参照してください。

## modifier の使用例

`modifer` を使うと字幕テキストを表示する前に改変することができます。  
ここでは字幕準備には `こんにちは` というテキストがセットされていると仮定します。

- `modifier = "名前\n"`<br>名前<br>こんにちは
- `modifier = function(t) return "名前「" .. t:match("^%s*(.-)%s*$") .. "」" end`<br>名前「こんにちは」
- `modifier = "<kern50>"`<br>&lt;kern50&gt;こんにちは
...
modifier については [modifier の使用例](#modifier_の使用例) を参照してください。  
上記以外の設定項目や効果については [テキストの装飾](decorate.md) を参照してください。

# 自動折り返しテキスト

`自動折り返しテキスト` は、一定以上の長さで自動的に折り返すことができる通常のテキストです。  
字幕ではないテキストで折り返しなどの機能が使いたい場合に有用です。

`自動折り返しテキスト` は拡張編集の右クリックメニューから `メディアオブジェクトの追加`→`PSDToolKit`→`自動折り返しテキスト` で作成できます。

テキスト入力欄にある設定は [`字幕表示（キャッシュ）`](#字幕表示（キャッシュ）) とほぼ同じです。  
設定項目について詳しくは [テキストの装飾](decorate.md) を参照してください。
