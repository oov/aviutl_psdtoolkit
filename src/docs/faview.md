...
プロパティ名|説明
---|---
`間隔`|次に瞬きを始めるまでのフレーム数です。<br>初期値は `30fps` の時に適度な値です。
`速さ`|瞬きの速さをフレーム単位で指定します。<br>初期値は `30fps` の時に適度な値です。
...
3. **`準備レイヤ` に `口パク準備` のレイヤー番号を指定**  
アニメーションの基準になる `口パク準備` が配置されているレイヤー番号を指定します。  
...
`準備レイヤ`|アニメーションの基準になる `口パク準備` が配置されているレイヤー番号を指定します。
...
既にタイムラインに設置済みの複数の `口パク準備` を[一括で設定変更する方法もあります](audio.md#自動作成される_口パク準備_のパラメーターの初期設定を変更する)。
...
local layer = 1 -- 準備レイヤ
local speed = 1 -- 速さ
local lipsync = require("PSDToolKit").LipSyncSimple.new(patterns, speed, layer)
...
細かい制御をしたい場合は `*.lab` ファイルを直接拡張編集のタイムライン上に投げ込むのがオススメです。  
...
local layer = 1 -- 準備レイヤ
local mode = 1 -- 子音処理
local lipsync = require("PSDToolKit").LipSyncLab.new(patterns, mode, layer)
PSD:addstate(lipsync)
```
...
ただし拡張編集の `パラメータ設定` ダイアログには文字数上限や項目数上限があるため、多数のパーツを一気に指定する用途には向きません。たくさんの項目から選びたい場合は[スライダーのエクスポート](#スライダーのエクスポート)の利用を検討してください。
...
目パチと同じように口パク系ももちろん登録できますが、口パクは同じやり方でやると `準備レイヤ` の指定を編集中に変更したりできず不便になるので、もう少し工夫するのがオススメです。

![`アニメーション効果` の前に `スクリプト制御` を挿入](assets/faview-export-track4.png)

このように `アニメーション効果` の前に `スクリプト制御` を挿入し `layer = 1` として準備レイヤの位置を指定しておいて、
...
アニメーション効果のスクリプト内では `layer` の値を参照するようにすれば、必要なタイミングで `layer = 1` の部分を書き換えるだけで `準備レイヤ` の指定を変更できるようになります。

もちろん `準備レイヤ` 以外のパラメーターも外部から指定できるようにしても便利ですし、`LipSyncSimple` を作成するコード自体をスクリプト制御側に移動すれば、煩雑になる代わりに柔軟に対応できるようになります。

# 項目名が暗号みたいなテキストになってしまうときは
...
`S_44GK5rCX44Gr5YWl44KKLyrjg4bjgrnjg4gv55yJfuihqOekuuOBp-OBjeOCi--8nw`

のような読めないテキストになってしまうことがあります。
