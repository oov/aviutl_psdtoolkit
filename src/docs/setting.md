...
`*.wav` ファイルをどのように投げ込むと `口パク準備` を作成するかを設定します。
...
値|説明
---|---
`0`|`*.wav` ファイルをエクスプローラーで掴んだあと、Shift キーを押しながら拡張編集に投げ込むと `口パク準備` を自動作成
`1`|`*.wav` ファイルをエクスプローラーで掴んだあと、Shift キーを押さずに拡張編集に投げ込んだ時、`*.wav` ファイルと拡張子だけが違う `*.txt` （つまり `こんにちは.wav` に対して `こんにちは.txt`）がある時は `口パク準備` を自動作成。ただし Shift キーを押しながら拡張編集に投げ込んだ場合は `口パク準備` を作成しない
