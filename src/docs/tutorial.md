...
音声ファイルの内容によっては口パク準備のプロパティを変更する必要がありますが、１つ１つに違う値を設定するのではなく一括で同じ値を設定したいときは、[`PSD ファイルオブジェクト` にある `ls_locut`/`ls_hicut`/`ls_threshold` の設定](obj.md#PSD_ファイルオブジェクト)を使うと一気に上書きできます。
