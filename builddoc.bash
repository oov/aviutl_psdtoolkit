...
pandoc.exe forcepser.md -o $OUTPUT_DIR/forcepser.html -f gfm -M curpage="forcepser" -M title="かんしくん" -M author="oov" -M date="$DATE" -M target="$TARGET" --lua-filter filter/remove-colgroup.lua --lua-filter filter/link-md2html.lua --template=template.html -s --toc --base-header-level=2 --toc-depth=5
...
pandoc.exe otheranm.md -o $OUTPUT_DIR/otheranm.html -f gfm -M curpage="otheranm" -M title="その他のアニメーション効果" -M author="oov" -M date="$DATE" -M target="$TARGET" --lua-filter filter/remove-colgroup.lua --lua-filter filter/link-md2html.lua --template=template.html -s --toc --base-header-level=2 --toc-depth=5
pandoc.exe plugins.md -o $OUTPUT_DIR/plugins.html -f gfm -M curpage="plugins" -M title="付属プラグイン" -M author="oov" -M date="$DATE" -M target="$TARGET" --lua-filter filter/remove-colgroup.lua --lua-filter filter/link-md2html.lua --template=template.html -s --toc --base-header-level=2 --toc-depth=5
